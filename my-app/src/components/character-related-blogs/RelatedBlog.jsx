...
const BlogItem = ({ blog, type }) => (
  <div className="group flex items-center gap-4 p-4 mb-4 rounded-xl bg-white shadow-sm hover:shadow-md transition-all duration-300 border border-gray-100 hover:border-purple-200">
...
export const RelatedBlog = ({
  ...
  if (loadType === "Icon") {
    return (
      <div className="h-[700px] mt-20 grid grid-cols-1 place-items-center gap-3 bg-gradient-to-b from-white to-gray-50 rounded-2xl p-6">
        {hasBlog && blogComic && <BlogIcon blog={blogComic} />}
        {Array.isArray(relatedCharacters)
          ? relatedCharacters?.map((char) => (
              <BlogIcon key={char.id} blog={char} />
            ))
          : relatedCharacters && <BlogIcon blog={relatedCharacters} />}
        {validate(relatedInsightBlogs) &&
          relatedInsightBlogs.map((blog) => (
            <BlogIcon key={blog.id} blog={blog} />
          ))}
